...
bcg_image = 'images/background.jpg'
snake_image = 'images/snake.png'
pygame.mixer.init()
pygame.mixer.music.load('sounds/EddieTalk.wav')
...
def main_gameplay():
    ...
    n_pip1 = get_Random_snakes()
    n_pip2 = get_Random_snakes()
    ...
    while True:
        ...
        for snake in up_pips:
            pip_middle_positions = snake['x'] + game_image['snake'][0].get_width() / 2
        ...
        if 0 < up_pips[0]['x'] < 5:
            new_pip = get_Random_snakes()
            up_pips.append(new_pip[0])
            low_pips.append(new_pip[1])


        if up_pips[0]['x'] < -game_image['snake'][0].get_width():
            up_pips.pop(0)
            low_pips.pop(0)
        ...
        display_screen_window.blit(game_image['background'], (0, 0))
        for pip_upper, pip_lower in zip(up_pips, low_pips):
            display_screen_window.blit(game_image['snake'][0], (pip_upper['x'], pip_upper['y']))
            display_screen_window.blit(game_image['snake'][1], (pip_lower['x'], pip_lower['y']))
...
def is_Colliding(p_x, p_y, up_snakes, low_snakes):
    if p_y > play_ground - 25 or p_y < 0:
        game_audio_sound['hit'].play()
        return True

    for snake in up_snakes:
        pip_h = game_image['snake'][0].get_height()
        if (p_y < pip_h + snake['y'] and abs(p_x - snake['x']) < game_image['snake'][0].get_width()):
            game_audio_sound['hit'].play()
            return True

    for snake in low_snakes:
        if (p_y + game_image['player'].get_height() > snake['y']) and abs(p_x - snake['x']) < \
                game_image['snake'][0].get_width():
            game_audio_sound['hit'].play()
            return True
...
def get_Random_snakes():
    """
    Generate positions of two snakes(one bottom straight and one top rotated ) for blitting on the screen
    """
    pip_h = game_image['snake'][0].get_height()
    off_s = scr_height / 3
    ...
    y1 = pip_h - yes2 + off_s
    snake = [
        {'x': snakeX, 'y': -y1},  # upper snake
        {'x': snakeX, 'y': yes2}  # lower snake
    ]
    return snake
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('Eddie Snakes Game')
    game_image['numbers'] = (
    ...
    game_image['snake'] = (pygame.transform.rotate(pygame.image.load(snake_image).convert_alpha(), 180),
                          pygame.image.load(snake_image).convert_alpha()
                          )
